...
import { ExpertTitleCard } from "./expert-title-card"

interface DroppableAccordionExpertItemProps {
    id: string
    title: string
    items: RankedExpert[]
    onChangeCategory?: (itemId: number, newCategory: string) => void
}

export function DroppableAccordionExpertItem({ id, title, items, onChangeCategory }: DroppableAccordionExpertItemProps) {
            ...
            >
                {items.length > 0 ? (
                    <div className="space-y-2">
                        {items.map((item) => (
                            <ExpertTitleCard key={item.expert_id} item={item} onChangeCategory={(newCategory) => {
                                if (onChangeCategory) {
                                    console.log("Changing category of", item.expert_id, "to", newCategory)
                                    onChangeCategory(item.expert_id, newCategory)
                                }
                            }} />
                        ))}
                    </div>
